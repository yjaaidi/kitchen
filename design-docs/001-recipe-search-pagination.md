...
- **`Paginator` component** (in `shared/`) — inputs: `offset`, `limit`, `total`; output: `offsetChange: number`. Computes `hasPrevious = offset > 0` and `hasNext = offset + limit < total` internally. Renders "Previous" and "Next" buttons, disabling each accordingly. Emits new offset on click.
- **`RecipeSearch` component** — adds an `offset` signal (resets to 0 on filter change). Passes `offset`, `PAGE_SIZE` (constant), and `total` from the response to `Paginator`. Shows a spinner while loading.
...
flowchart TD
    ...
    PAG["Paginator<br>[offset: number]<br>[limit: number]<br>[total: number]<br>(offsetChange: number)"]
...
- **PR 1 — Scaffold `Paginator` component**: Create the `Paginator` component in `shared/` with inputs (`offset`, `limit`, `total`), output (`offsetChange`), and tests. No integration with `RecipeSearch` yet.
...
- **PR 3 — Wire pagination into `RecipeSearch`**: Add `offset` signal, integrate `Paginator` component, reset offset on filter change, add spinner. Update `RecipeSearch` tests.
